function [acc_train,acc_test,time] = main_RR_Multi_view(X,trainIdx,testIdx,num_idx,lamda)
%{
****武林璐
****2017-11-22 晚
%}
...
for num_view = 1:v
    ...
    TrnData = bsxfun(@minus, TrnData1, mean(TrnData1,2));     %训练测试数据分别去中心化
    TestData = bsxfun(@minus, TestData1, mean(TrnData1,2));

    TrainingData_File = [TrnLabels TrnData'];                 %组装数据每行是一个样本
    TestingData_File = [TestLabels TestData'];

    tic 
    [test_result,train_result] = RR(TrainingData_File, TestingData_File,lamda);
    tmp = toc;
